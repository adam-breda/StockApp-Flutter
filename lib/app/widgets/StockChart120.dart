...
class StockChart120 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AspectRatio(
      aspectRatio: 2,
      child: Obx(
        () => controller.historicalData120.length > 0
            ? LineChart(
                LineChartData(
                  ...
                  minX: 0,
                  maxX: controller.historicalData120.length.toDouble() - 1,
                  minY: getMinVolume(),
                  maxY: getMaxVolume(),
                  lineBarsData: [
                    LineChartBarData(
                  ...
                  titlesData: FlTitlesData(
                    bottomTitles: SideTitles(
                      ...
                      getTitles: (value) {
                        ...
                        if (index >= 0 &&
                            index < controller.historicalData120.length) {
                          final date = DateTime.parse(
                              controller.historicalData120[index].date);
  ...
  double getMinVolume() {
    ...
    for (var data in controller.historicalData120) {
  ...
  List<FlSpot> getChartData() {
    ...
    for (int i = 0; i < controller.historicalData120.length; i++) {
      chartData.add(FlSpot(
          i.toDouble(), controller.historicalData120[i].volume.toDouble()));
    }
    return chartData;
  }
}
