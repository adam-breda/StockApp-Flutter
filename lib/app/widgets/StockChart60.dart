...
class StockChart60 extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return AspectRatio(
      aspectRatio: 2,
      child: Obx(
        () => controller.historicalData60.length > 0
            ? LineChart(
                LineChartData(
                  ...
                  minX: 0,
                  maxX: controller.historicalData60.length.toDouble() - 1,
                  minY: getMinVolume(),
                  maxY: getMaxVolume(),
                  lineBarsData: [
                    LineChartBarData(
                  ...
                  titlesData: FlTitlesData(
                    bottomTitles: SideTitles(
                      ...
                      getTitles: (value) {
                        ...
                        if (index >= 0 &&
                            index < controller.historicalData60.length) {
                          final date = DateTime.parse(
                              controller.historicalData60[index].date);
  ...
  double getMinVolume() {
    ...
    for (var data in controller.historicalData60) {
  ...
  List<FlSpot> getChartData() {
    ...
    for (int i = 0; i < controller.historicalData60.length; i++) {
      chartData.add(FlSpot(
          i.toDouble(), controller.historicalData60[i].volume.toDouble()));
    }
    return chartData;
  }
}
