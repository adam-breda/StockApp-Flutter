...
class DetailPerformance extends StatelessWidget {
  ...
  List<FlSpot> getChartDataROE() {
    List<FlSpot> chartData = [];
    for (int i = 0; i < controller.DetailValuation.length; i++) {
      chartData.add(FlSpot(i.toDouble(),
          controller.DetailValuation[i].annualizedRoe?.toDouble() ?? 0.0));
    }
  ...
  List<FlSpot> getChartDataROA() {
    List<FlSpot> chartDataROA = [];
    for (int i = 0; i < controller.DetailValuation.length; i++) {
      chartDataROA.add(FlSpot(i.toDouble(),
          controller.DetailValuation[i].annualizedRoa?.toDouble() ?? 0.0));
    }
    print("ini chart data");
    print(chartDataROA);
    return chartDataROA;
